...
// LuckAs add Serial from Wiko - IQ442
#define CONFIG_MTK_USB_UNIQUE_SERIAL
// LuckAs end
...
// LuckAs add for BCM from Wiko IQ442
...
// LuckAs add for Serial from Wiko-IQ442
...
// LuckAs disable I2C - change from Wiko IQ442, FIXME!!! 
...
// LuckAs end
    ...
    for (i = 0; i < ARRAY_SIZE(mt6575_device_sd); i++){
        retval = platform_device_register(&mt6575_device_sd[i]);
			if (retval != 0){
// LuckAs add from Wiko IQ442
				printk("[%s]: failed to register mt6575_device_sd[%d]\n", __func__, i);
// LuckAs end

				return retval;
			}
// LuckAs add from Wiko IQ442
			printk("[%s]: successed to register mt6575_device_sd[%d]\n", __func__, i);
// LuckAs end
		}
#endif
...
//[WJ ADD]
	...
	if (retval != 0){
		printk("platform_device_brcm_bluesleep_register error:(%d)\n", retval);	
		return retval;
	}
#endif //CONFIG_BT_BRCM_SLEEP
// Luckas end

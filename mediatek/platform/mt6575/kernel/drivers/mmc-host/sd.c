...
void msdc_clk_status(int * status)
{
    ...
    for(i=0; i<HOST_MAX_NUM; i++)
    {
       ...
       spin_lock_irqsave(&msdc_6575_host[i]->clk_gate_lock, flags);
       if(msdc_6575_host[i]->clk_gate_count > 0)
         g_clk_gate |= 1 << ((i) + PERI_MSDC0_PDN);
       spin_unlock_irqrestore(&msdc_6575_host[i]->clk_gate_lock, flags);
    }
    *status = g_clk_gate;    	
}
    ...
    if (!data) {
        ...
        if (dma) {
			...
			if(host->id == EMMC_HOST_ID || host->id == SD_HOST_ID){
				sdr_get_field(SDC_CFG ,SDC_CFG_BUSWIDTH ,bus_mode); //MT6575 work arroud for UHS-1 SDR104 
            ...
            sdr_get_field(SDC_CFG ,SDC_CFG_BUSWIDTH ,bus_mode); //MT6575 work arroud for UHS-1 SDR104 
            if(bus_mode == 1 && host->sclk > 100000000)
		          sdr_set_field(MSDC_PATCH_BIT0, 1 << 16, 1);//MT6575 work arroud for UHS-1 SDR104 
            msdc_dma_start(host);
...
stop:        
        /* Last: stop transfer */
        sdr_set_field(MSDC_PATCH_BIT0, 1 << 16, 0);//MT6575 work arroud for UHS-1 SDR104 
        if (data->stop){ 
			  ...
			  int *eof, void *data_unused)
{
	...
	host = msdc_6575_host[EMMC_HOST_ID];
	BUG_ON(!host);
	BUG_ON(!host->mmc);
	BUG_ON(!host->mmc->card);

# orig
#AUTO_ADD_GLOBAL_DEFINE_BY_NAME = MTK_SHARED_SDCARD MTK_TB_APP_CALL_FORCE_SPEAKER_ON MTK_TABLET_PLATFORM MTK_LCA_SUPPORT MTK_WAPI_SUPPORT MTK_SENSOR_SUPPORT CUSTOM_KERNEL_ALSPS CUSTOM_KERNEL_MAGNETOMETER CUSTOM_KERNEL_ACCELEROMETER MTK_TVOUT_SUPPORT MTK_BT_SUPPORT MTK_FM_SUPPORT MTK_CAMERA_APP_3DHW_SUPPORT MTK_QVGA_LANDSCAPE_SUPPORT MTK_WLANBT_SINGLEANT MTK_FM_TX_SUPPORT MTK_VT3G324M_SUPPORT MTK_M4U_SUPPORT MTK_MATV_ANALOG_SUPPORT MTK_WLAN_SUPPORT MTK_BT_21_SUPPORT MTK_BT_30_SUPPORT MTK_BT_40_SUPPORT MTK_BT_FM_OVER_BT_VIA_CONTROLLER MTK_BT_PROFILE_OPP MTK_BT_PROFILE_SIMAP MTK_BT_PROFILE_PRXM MTK_BT_PROFILE_PRXR MTK_BT_PROFILE_HIDH MTK_BT_PROFILE_FTP MTK_BT_PROFILE_PBAP MTK_BT_PROFILE_MANAGER MTK_BT_PROFILE_BPP MTK_BT_PROFILE_BIP MTK_BT_PROFILE_DUN MTK_BT_PROFILE_PAN MTK_BT_PROFILE_HFP MTK_BT_PROFILE_A2DP MTK_BT_PROFILE_AVRCP MTK_BT_PROFILE_AVRCP14 MTK_BT_PROFILE_TIMEC MTK_BT_PROFILE_TIMES MTK_BT_PROFILE_MAPS MTK_BT_PROFILE_MAPC MTK_BT_PROFILE_SPP MTK_FM_SHORT_ANTENNA_SUPPORT MTK_DUAL_MIC_SUPPORT MTK_WIFI_HOTSPOT_SUPPORT MTK_CTA_SUPPORT MTK_FM_SUPPORT MTK_GPS_SUPPORT MTK_DSPIRDBG HAVE_XLOG_FEATURE MTK_AUTO_DETECT_MAGNETOMETER MTK_AUTO_DETECT_ACCELEROMETER MTK_MAV_SUPPORT MTK_HDR_SUPPORT MTK_CAMERA_BSP_SUPPORT CUSTOM_KERNEL_GYROSCOPE MTK_AUTORAMA_SUPPORT MTK_IPV6_SUPPORT MTK_MULTI_STORAGE_SUPPORT MTK_FM_RECORDING_SUPPORT MTK_USES_HD_VIDEO MTK_NFC_SUPPORT MTK_THEMEMANAGER_APP MTK_BRAZIL_CUSTOMIZATION MTK_BRAZIL_CUSTOMIZATION_CLARO MTK_BRAZIL_CUSTOMIZATION_VIVO MTK_RMVB_PLAYBACK_SUPPORT MTK_ASF_PLAYBACK_SUPPORT MTK_EMMC_SUPPORT MTK_HDMI_SUPPORT MODEM_UMTS_TDD128_MODE MTK_EAP_SIM_AKA MTK_FACEBEAUTY_SUPPORT MTK_FM_RX_SUPPORT MTK_TB_DEBUG_SUPPORT MTK_FAN5405_SUPPORT MTK_COMBO_SUPPORT MTK_CAMCORDER_PROFILE_MID_MP4 MTK_DISPLAY_HIGH_RESOLUTION MTK_FD_SUPPORT MTK_MASS_STORAGE MTK_FM_50KHZ_SUPPORT MTK_EMMC_SUPPORT_OTP MTK_2SDCARD_SWAP MTK_SD_REINIT_SUPPORT
# LuckAs change for BCM
AUTO_ADD_GLOBAL_DEFINE_BY_NAME = MTK_SHARED_SDCARD MTK_TB_APP_CALL_FORCE_SPEAKER_ON MTK_TABLET_PLATFORM MTK_LCA_SUPPORT MTK_WAPI_SUPPORT MTK_SENSOR_SUPPORT CUSTOM_KERNEL_ALSPS CUSTOM_KERNEL_MAGNETOMETER CUSTOM_KERNEL_ACCELEROMETER MTK_TVOUT_SUPPORT MTK_BT_SUPPORT MTK_FM_SUPPORT MTK_CAMERA_APP_3DHW_SUPPORT MTK_QVGA_LANDSCAPE_SUPPORT MTK_WLANBT_SINGLEANT MTK_FM_TX_SUPPORT MTK_VT3G324M_SUPPORT MTK_M4U_SUPPORT MTK_MATV_ANALOG_SUPPORT MTK_WLAN_SUPPORT MTK_BT_21_SUPPORT MTK_BT_30_SUPPORT MTK_BT_40_SUPPORT MTK_BT_FM_OVER_BT_VIA_CONTROLLER MTK_BT_PROFILE_OPP MTK_BT_PROFILE_SIMAP MTK_BT_PROFILE_PRXM MTK_BT_PROFILE_PRXR MTK_BT_PROFILE_HIDH MTK_BT_PROFILE_FTP MTK_BT_PROFILE_PBAP MTK_BT_PROFILE_MANAGER MTK_BT_PROFILE_BPP MTK_BT_PROFILE_BIP MTK_BT_PROFILE_DUN MTK_BT_PROFILE_PAN MTK_BT_PROFILE_HFP MTK_BT_PROFILE_A2DP MTK_BT_PROFILE_AVRCP MTK_BT_PROFILE_AVRCP14 MTK_BT_PROFILE_TIMEC MTK_BT_PROFILE_TIMES MTK_BT_PROFILE_MAPS MTK_BT_PROFILE_MAPC MTK_BT_PROFILE_SPP MTK_FM_SHORT_ANTENNA_SUPPORT MTK_DUAL_MIC_SUPPORT MTK_WIFI_HOTSPOT_SUPPORT MTK_CTA_SUPPORT MTK_FM_SUPPORT MTK_GPS_SUPPORT MTK_DSPIRDBG HAVE_XLOG_FEATURE MTK_AUTO_DETECT_MAGNETOMETER MTK_AUTO_DETECT_ACCELEROMETER MTK_MAV_SUPPORT MTK_HDR_SUPPORT MTK_CAMERA_BSP_SUPPORT CUSTOM_KERNEL_GYROSCOPE MTK_AUTORAMA_SUPPORT MTK_IPV6_SUPPORT MTK_MULTI_STORAGE_SUPPORT MTK_FM_RECORDING_SUPPORT MTK_USES_HD_VIDEO MTK_NFC_SUPPORT MTK_THEMEMANAGER_APP MTK_BRAZIL_CUSTOMIZATION MTK_BRAZIL_CUSTOMIZATION_CLARO MTK_BRAZIL_CUSTOMIZATION_VIVO MTK_RMVB_PLAYBACK_SUPPORT MTK_ASF_PLAYBACK_SUPPORT MTK_EMMC_SUPPORT MTK_HDMI_SUPPORT MODEM_UMTS_TDD128_MODE MTK_EAP_SIM_AKA MTK_FACEBEAUTY_SUPPORT MTK_FM_RX_SUPPORT MTK_TB_DEBUG_SUPPORT MTK_FAN5405_SUPPORT MTK_COMBO_SUPPORT MTK_CAMCORDER_PROFILE_MID_MP4 MTK_DISPLAY_HIGH_RESOLUTION MTK_FD_SUPPORT MTK_MASS_STORAGE MTK_FM_50KHZ_SUPPORT MTK_EMMC_SUPPORT_OTP MTK_2SDCARD_SWAP MTK_SD_REINIT_SUPPORT BCM_GPS_SUPPORT BCM_BT_SUPPORT BCM_WLAN_SUPPORT BCM_FM_SUPPORT
# LuckAs end

AUTO_ADD_GLOBAL_DEFINE_BY_VALUE = MTK_PLATFORM MTK_FM_CHIP MTK_BT_CHIP CUSTOM_KERNEL_MAIN_IMGSENSOR CUSTOM_KERNEL_MAIN_BACKUP_IMGSENSOR CUSTOM_KERNEL_SUB_IMGSENSOR CUSTOM_KERNEL_SUB_BACKUP_IMGSENSOR CUSTOM_KERNEL_MAIN_LENS CUSTOM_KERNEL_MAIN_BACKUP_LENS CUSTOM_KERNEL_SUB_LENS CUSTOM_KERNEL_SUB_BACKUP_LENS MTK_ATV_CHIP MTK_GPS_CHIP MTK_WLAN_CHIP MTK_MODEM_SUPPORT CUSTOM_KERNEL_LCM CUSTOM_KERNEL_FLASHLIGHT MTK_FM_TX_AUDIO MTK_FM_RX_AUDIO MTK_COMBO_CHIP

AUTO_ADD_GLOBAL_DEFINE_BY_NAME_VALUE = LCM_WIDTH LCM_HEIGHT MTK_LCM_PHYSICAL_ROTATION GEMINI MTK_GEMINI_ENHANCEMENT MTK_GEMINI_3G_SWITCH MTK_SHARE_MODEM_SUPPORT MTK_SHARE_MODEM_CURRENT EMMC_CHIP

MTK_PLATFORM = MT6575

# LuckAs add from iq442
CUSTOM_BUILD_VERNO=LuckAs_S8081_V1.0
#CUSTOM_DRAM_SIZE = 3G
#MTK_MTD_NAND = yes
#MTK_LDVT_SUPPORT = no
#MTK_EMMC_OTP_SUPPORT = no
#MTK_NEON_SUPPORT = yes
#MTK_SWIP_VORBIS = yes
#EVDO_DT_SUPPORT = no
#EVDO_DT_VIA_SUPPORT = no
#GOOGLE_RELEASE_RIL = no
#MTK_ACMT_DEBUG = no
#MTK_DT_SUPPORT = no
#MTK_RCSE_SUPPORT = no
#MTK_WCDMA_SUPPORT = no
# LuckAs end

#
# MTK Build System
#
BUILD_KERNEL = yes
	# default settings: yes for target; else no (e.g. emulator)
	# candidate settings: yes; no
	# used for MTK build system

BUILD_PRELOADER = yes
	# default settings: yes for target; else no (e.g. emulator)
	# candidate settings: yes; no
	# used for MTK build system

BUILD_UBOOT = yes
	# default settings: yes for target; else no (e.g. emulator)
	# candidate settings: yes; no
	# used for MTK build system

#
# Customize for PRELOADER
#
CUSTOM_PRELOADER_CUSTOM = custom

#
# LCD and LCM
#
CUSTOM_UBOOT_LCM = nt35310_dbi_18bits_tinno
#CUSTOM_UBOOT_LCM=nt35310_dbi_18bits_tinno r61581 xxd35_ili9486l
#CUSTOM_UBOOT_LCM=nt35310_dbi_18bits_tinno ili9486l_18bits r61581 xxd35_ili9486l

# try
CUSTOM_KERNEL_LCM = nt35310_boe_lt3526 ili9486l_18bits
# work 1-rev but strange colors but no mirror (ok) 2-rev not tested
#CUSTOM_KERNEL_LCM = nt35310_dbi_18bits_tinno ili9486_hvga
# works one for two!!! strange colors and mirror for 1-rev, 2-rev all OK!
#CUSTOM_KERNEL_LCM = nt35310_dbi_18bits_tinno ili9486l_18bits
# works 2-revision but bad
#CUSTOM_KERNEL_LCM = nt35310_dbi_18bits_tinno ili9481_18bits
# work fine!
#CUSTOM_KERNEL_LCM = nt35310_dbi_18bits_tinno
#CUSTOM_KERNEL_LCM=nt35310_dbi_18bits_tinno r61581 xxd35_ili9486l
#CUSTOM_KERNEL_LCM=nt35310_dbi_18bits_tinno ili9486l_18bits r61581 xxd35_ili9486l

LCM_WIDTH = 320
LCM_HEIGHT = 480
BOOT_LOGO = cu_hvga
MTK_LCM_PHYSICAL_ROTATION = 0
  # set this value as 90 or 180 if the lcm is physically rotated on the device

#
# EMMC or NAND: 
#
MTK_EMMC_SUPPORT = no
EMMC_CHIP = 
MTK_EMMC_SUPPORT_OTP = no
MTK_NAND_PAGE_SIZE = 2K
CONFIG_MTK_MTD_NAND = yes

#
# Core: 
#

# LuckAs change no eeprom
CUSTOM_HAL_EEPROM = 
CUSTOM_KERNEL_EEPROM = 
# tooky
#CUSTOM_HAL_EEPROM = dummy_eeprom
#CUSTOM_KERNEL_EEPROM = dummy_eeprom
CUSTOM_KERNEL_KPD = kpd
CUSTOM_KERNEL_LEDS = mt65xx
CUSTOM_KERNEL_USB = mt6575
CUSTOM_KERNEL_BATTERY = battery
MTK_FAN5405_SUPPORT = no
CUSTOM_KERNEL_VIBRATOR = vibrator
CUSTOM_KERNEL_CORE = src
CUSTOM_KERNEL_DCT = dct
CUSTOM_KERNEL_RTC = rtc
MTK_GPU_CHIP = SGX351_110

#
# Sensors: 
#
MTK_SENSOR_SUPPORT = yes
MTK_AUTO_DETECT_ACCELEROMETER = yes
MTK_AUTO_DETECT_MAGNETOMETER = no

CUSTOM_HAL_SENSORS = sensor
CUSTOM_HAL_MSENSORLIB = bmm050 mmc328x akm8975 ami304 yamaha530 mag3110 akmd8963

# LuckAs temp disable mag
CUSTOM_KERNEL_MAGNETOMETER = 
#CUSTOM_KERNEL_MAGNETOMETER = bmm050
CUSTOM_KERNEL_ACCELEROMETER = bma222E_auto
# all
#CUSTOM_KERNEL_ACCELEROMETER = bma250_auto bma222E_auto kxtj2_auto

# LuckAs need for sales
CUSTOM_KERNEL_ALSPS = 
#CUSTOM_KERNEL_ALSPS = cm3607

#CUSTOM_KERNEL_TOUCHPANEL = msg2133_a350
CUSTOM_KERNEL_TOUCHPANEL = msg2133_ps_iq445
#CUSTOM_KERNEL_TOUCHPANEL = msg2133
CUSTOM_KERNEL_GYROSCOPE =
CUSTOM_KERNEL_BAROMETER =

#
# Camera and periphery for camera
#
CUSTOM_HAL_CAMERA = camera
CUSTOM_KERNEL_CAMERA = camera

CUSTOM_HAL_IMGSENSOR = ov2659_yuv hi253_yuv hi704_yuv
CUSTOM_KERNEL_IMGSENSOR = ov2659_yuv hi253_yuv hi704_yuv
# orig
#CUSTOM_KERNEL_IMGSENSOR = ov2659_yuv hi253_yuv hi704_yuv
# firstly
#CUSTOM_HAL_IMGSENSOR = hi253_yuv ov2659_yuv hi704_yuv
#CUSTOM_KERNEL_IMGSENSOR = hi253_yuv ov2659_yuv hi704_yuv

CUSTOM_HAL_MAIN_IMGSENSOR = hi253_yuv
CUSTOM_HAL_MAIN_BACKUP_IMGSENSOR = ov2659_yuv
CUSTOM_HAL_SUB_IMGSENSOR = hi704_yuv
CUSTOM_HAL_SUB_BACKUP_IMGSENSOR =

CUSTOM_KERNEL_MAIN_IMGSENSOR = ov2659_yuv
#CUSTOM_KERNEL_MAIN_BACKUP_IMGSENSOR = 
# orig
CUSTOM_KERNEL_MAIN_BACKUP_IMGSENSOR = hi253_yuv
CUSTOM_KERNEL_SUB_IMGSENSOR = hi704_yuv
# firstly
#CUSTOM_KERNEL_MAIN_IMGSENSOR = hi253_yuv
#CUSTOM_KERNEL_MAIN_BACKUP_IMGSENSOR = ov2659_yuv
#CUSTOM_KERNEL_SUB_IMGSENSOR = hi704_yuv
CUSTOM_KERNEL_SUB_BACKUP_IMGSENSOR =

# Flash LED for camera
CUSTOM_HAL_FLASHLIGHT = constant_flashlight
CUSTOM_KERNEL_FLASHLIGHT = dummy_flashlight
# old
#CUSTOM_KERNEL_FLASHLIGHT = constant_flashlight

# Motor sensor for AF camera
CUSTOM_HAL_LENS = fm50af dummy_lens
CUSTOM_KERNEL_LENS = fm50af dummy_lens

CUSTOM_HAL_MAIN_LENS = fm50af
CUSTOM_HAL_MAIN_BACKUP_LENS = 
CUSTOM_HAL_SUB_LENS = dummy_lens
CUSTOM_HAL_SUB_BACKUP_LENS =

CUSTOM_KERNEL_MAIN_LENS = fm50af
CUSTOM_KERNEL_MAIN_BACKUP_LENS = 
CUSTOM_KERNEL_SUB_LENS = dummy_lens
CUSTOM_KERNEL_SUB_BACKUP_LENS =

#
# Multimedia: 
#
CUSTOM_HAL_AUDIOFLINGER = audio
CUSTOM_HAL_BLUETOOTH = bluetooth
CUSTOM_KERNEL_HEADSET = accdet
CUSTOM_KERNEL_SOUND = amp_6329pmic_spk
CUSTOM_KERNEL_HDMI =

# LuckAs try MATV FIXME!!!
#CUSTOM_HAL_MATV = matv
#CUSTOM_KERNEL_MATV = mt5193
#MTK_MATV_ANALOG_SUPPORT = yes
#HAVE_MATV_FEATURE = yes
# add
#MTK_MT519X_FM_SUPPORT= yes
# orig
CUSTOM_HAL_MATV = 
CUSTOM_KERNEL_MATV = 
HAVE_MATV_FEATURE = no
MTK_MATV_ANALOG_SUPPORT = yes
   # For I2S, use: no
   # For Audio Line-in, use: yes
# LuckAs disable FIXME!!!
#MTK_ATV_CHIP = MTK_MT5192

# Enable dual mic support
MTK_DUAL_MIC_SUPPORT = no
MTK_HDMI_SUPPORT = no

#
# Combo chip, WIFI, FM, BT, GPS, NFC: 
#
CUSTOM_KERNEL_WIFI =
CUSTOM_HAL_ANT = 
# orig
#CUSTOM_HAL_ANT = mt6620_ant_m3

MTK_WLANBT_SINGLEANT = no

CUSTOM_HAL_COMBO = 
# orig
#CUSTOM_HAL_COMBO = mt6620

# LuckAs below is present
#MTK_BT_CHIP = BCM4330
# orig
#MTK_BT_CHIP = MTK_MT6620

MTK_CHIP_VER = S01

# LuckAs add
# Disable MTK FM and enable Broadcom FM
BCM_FM_SUPPORT = yes
MTK_FMRADIO_APP = no
# orig
#MTK_FMRADIO_APP = yes
MTK_FM_CHIP = BCM4330_FM
# orig
#MTK_FM_CHIP = MT6620_FM
MTK_FM_RECORDING_SUPPORT = no
# orig
#MTK_FM_RECORDING_SUPPORT = yes
MTK_FM_RX_SUPPORT = no
# orig
#MTK_FM_RX_SUPPORT = yes
MTK_FM_SHORT_ANTENNA_SUPPORT = no
# orig
#MTK_FM_SHORT_ANTENNA_SUPPORT = yes
MTK_FM_SUPPORT = no
# orig
#MTK_FM_SUPPORT = yes
MTK_FM_TX_SUPPORT = no
# orig
#MTK_FM_TX_SUPPORT = yes

# check it fixme!
MTK_FM_TX_AUDIO = 
# orig
#MTK_FM_TX_AUDIO = FM_ANALOG_OUTPUT
CUSTOM_KERNEL_FM = 
# orig
#CUSTOM_KERNEL_FM = mt6620

# LuckAs add
# Disable MTK Combo chip
MTK_COMBO_CHIP = 
MTK_COMBO_SUPPORT = no
# orig
#MTK_COMBO_CHIP = MT6620
#MTK_COMBO_SUPPORT = yes

# Disable MTK WiFi and enable Broadcom WiFi
BCM_WLAN_SUPPORT = yes
MTK_WLAN_SUPPORT = no
# orig
#MTK_WLAN_SUPPORT = yes
MTK_WLAN_CHIP = 
# orig
#MTK_WLAN_CHIP = MT6620

MTK_WIFI_P2P_SUPPORT = no
# orig 
#MTK_WIFI_P2P_SUPPORT = yes

MTK_WIFI_HOTSPOT_SUPPORT = no

MTK_WAPI_SUPPORT = no
# orig
#MTK_WAPI_SUPPORT = yes

# Disable MTK GPS and enable Broadcom GPS
BCM_GPS_SUPPORT = yes
MTK_GPS_SUPPORT = no
# orig
#MTK_GPS_SUPPORT = yes
MTK_GPS_CHIP = 
# orig
#MTK_GPS_CHIP = MTK_GPS_MT6620


MTK_BT_SUPPORT = no
# orig
#MTK_BT_SUPPORT = yes

MTK_NFC_SUPPORT = no

# LuckAs think ...
MTK_MT519X_FM_SUPPORT = no

# LuckAs check it fixme!!!
#MTK_FM_RX_AUDIO = 
# orig
MTK_FM_RX_AUDIO = FM_ANALOG_INPUT

#
# Internet settings?
#

# LuckAs check it fixme!!!
MTK_WPA2PSK_SUPPORT = yes

# LuckAs check it fixme!!!
MTK_WAPPUSH_SUPPORT = yes

MTK_IPO_SUPPORT = no
# orig 
#MTK_IPO_SUPPORT = yes

# LuckAs check it
MTK_IPV6_SUPPORT = yes

MTK_TTY_SUPPORT = yes

MTK_NEW_IPTABLES_SUPPORT = yes

#
# Other: 
#
CUSTOM_KERNEL_JOGBALL =
CUSTOM_KERNEL_OFN =
CUSTOM_KERNEL_CMMB =
MTK_CMMB_CHIP = Siano
    #   Siano              :  Use siano
	#	Innofidei_IF228_20M:  Use IF228 and clk 20M
    #   Innofidei_IF228_26M:  Use IF228 and clk 26M
	#	Innofidei_IF238_20M:  Use IF238 and clk 20M
	#	Innofidei_IF238_26M:  Use IF238 and clk 26M
	#   NONE:       Default Use Siano


#
# Support modem and 3G: 
#
CUSTOM_MODEM = tinnoes75_s8053_hspa_sp
EVB = no
GEMINI = yes
MTK_GEMINI_ENHANCEMENT = yes
OPTR_SPEC_SEG_DEF = NONE	
MTK_GEMINI_3G_SWITCH = yes
MTK_SHARE_MODEM_SUPPORT = 2
MTK_SHARE_MODEM_CURRENT = 2

# LuckAs check it fixme!!!
MTK_VT3G324M_SUPPORT = yes
MTK_MODEM_SUPPORT = modem_3g

# LuckAs check it fixme!!!
MTK_FD_SUPPORT = yes

# LuckAs check it fixme!!!
MTK_FD_FORCE_REL_SUPPORT = yes

# LuckAs check it fixme!!!
MODEM_UMTS_TDD128_MODE = no

#
# MTK Specified Feature
#
MTK_M4U_SUPPORT = yes
HAVE_AEE_FEATURE = yes
HAVE_APPC_FEATURE = no


HAVE_GROUP_SCHEDULING = no
HAVE_CMMB_FEATURE = no
HAVE_XLOG_FEATURE = yes
HAVE_AWBENCODE_FEATURE = yes
HAVE_AACENCODE_FEATURE = yes
HAVE_VORBISENC_FEATURE = yes
HAVE_SRSAUDIOEFFECT_FEATURE = no

#
# MTK storage support
#
MTK_2SDCARD_SWAP = yes

# LuckAs add from iq442
MTK_SWAP_STATIC_MODE=yes

# LuckAs check it fixme!!!
# FIXME!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
# maybe BUG at WiFi
#MTK_SD_REINIT_SUPPORT = yes
# orig
MTK_SD_REINIT_SUPPORT = no

MTK_MASS_STORAGE = yes

# LuckAs check it fixme!!!
MTK_MULTI_STORAGE_SUPPORT = yes

# LuckAs check it fixme!!!
MTK_SHARED_SDCARD = no

#
# MTK SUPPORT
#

# LuckAs check it fixme!!!
MTK_TVOUT_SUPPORT = no

MTK_LCA_SUPPORT = no
MTK_CAMERA_APP_3DHW_SUPPORT = no
MTK_QVGA_LANDSCAPE_SUPPORT = no
MTK_SCREEN_OFF_WIFI_OFF = no

# LuckAs check it fixme!!!
MTK_HEADSET_ICON_SUPPORT = no

MTK_OMA_DOWNLOAD_SUPPORT = yes
MTK_SMARTSWITCH_SUPPORT = no
MTK_OMACP_SUPPORT = yes

MTK_GOOGLEOTA_SUPPORT = yes
# orig
#MTK_GOOGLEOTA_SUPPORT = no

# LuckAs add from 442
MTK_LAUNCHER_UNREAD_SUPPORT = yes

MTK_SEARCH_DB_SUPPORT = yes
MTK_DIALER_SEARCH_SUPPORT = yes
MTK_MDLOGGER_SUPPORT = yes
MTK_FOTA_SUPPORT = no
MTK_MUSIC_LRC_SUPPORT = no
MTK_MAV_SUPPORT = yes
MTK_HDR_SUPPORT = yes
MTK_CAMERA_BSP_SUPPORT = yes
MTK_DITHERING_SUPPORT = yes
MTK_AUTORAMA_SUPPORT = yes
MTK_WB_SPEECH_SUPPORT = yes
MTK_EMULATOR_SUPPORT = no
MTK_ASD_SUPPORT = yes
MTK_AVI_PLAYBACK_SUPPORT = yes
MTK_AUDIO_APE_SUPPORT = yes
MTK_WML_SUPPORT = yes
MTK_FLV_PLAYBACK_SUPPORT = no
MTK_RMVB_PLAYBACK_SUPPORT = no

MTK_ASF_PLAYBACK_SUPPORT = no
# orig
#MTK_ASF_PLAYBACK_SUPPORT = yes

MTK_OGM_PLAYBACK_SUPPORT = no
MTK_MTKPS_PLAYBACK_SUPPORT = no
MTK_SEND_RR_SUPPORT = yes
MTK_FACEBEAUTY_SUPPORT = yes
MTK_TB_DEBUG_SUPPORT = no
MTK_NOTEBOOK_SUPPORT = no
MTK_RTSP_BITRATE_ADAPTATION_SUPPORT = no
MTK_LOCKSCREEN_TYPE = 2
  # default=1, slide=2

#
# Miscl
#

RESOURCE_OVERLAY_SUPPORT = generic
# orig
#RESOURCE_OVERLAY_SUPPORT =

MTK_75DISPLAY_ENHANCEMENT_SUPPORT =no
MTK_DISPLAY_HIGH_RESOLUTION = no
DEFAULT_INPUT_METHOD = com.sohu.inputmethod.sogou.SogouIME
DEFAULT_LATIN_IME_LANGUAGES = en_US
MTK_PRODUCT_LOCALES=en_US es_ES zh_CN zh_TW ru_RU pt_BR fr_FR de_DE tr_TR it_IT in_ID ms_MY vi_VN ar_EG  th_TH  pt_PT   nl_NL el_GR hu_HU tl_PH ro_RO cs_CZ hdpi mdpi ldpi

MTK_RELEASE_PACKAGE=rel_customer_basic 
# orig
#MTK_RELEASE_PACKAGE = rel_customer_basic rel_customer_basic_mt6575 rel_customer_operator_cu

MTK_MFV_MPEG4_EXTRA = no
MTK_RESOURCE_OPTIMIZATION = 
MTK_INTERNAL = no
MTK_INTERNAL_LANG_SET = no
MTK_DSPIRDBG = no
MTK_SEC_USBDL = ATTR_SUSBDL_ONLY_ENABLE_ON_SCHIP
MTK_SEC_CHIP_SUPPORT = yes
MTK_SEC_BOOT = ATTR_SBOOT_ONLY_ENABLE_ON_SCHIP
MTK_SEC_MODEM_ENCODE = no
MTK_SEC_MODEM_AUTH = no
MTK_SEC_SECRO_AC_SUPPORT = no
CUSTOM_SEC_SIGNTOOL_SUPPORT = no
CUSTOM_SEC_AUTH_SUPPORT = no
MTK_PHONE_VT_MM_RINGTONE = no
MTK_PHONE_VT_VOICE_ANSWER = no
MTK_PHONE_VOICE_RECORDING = yes
MTK_FOTA_ENTRY = no
MTK_SCOMO_ENTRY = no
MTK_DM_ENTRY_DISPLAY = no
MTK_RTP_OVER_RTSP_SUPPORT = yes
MTK_SPECIAL_FACTORY_RESET = no
MTK_USES_STAGEFRIGHT_DEFAULT_CODE = no

MTK_EAP_SIM_AKA = yes

MTK_USES_HD_VIDEO = yes
MTK_SIGNATURE_CUSTOMIZATION = no
MTK_TB_APP_LANDSCAPE_SUPPORT = no
MTK_LAUNCHER_ALLAPPSGRID = yes
MTK_DEFAULT_DATA_OFF = no
MTK_TABLET_PLATFORM = no
MTK_TB_APP_CALL_FORCE_SPEAKER_ON = no
MTK_PDP_WHEN_NEEDED = yes
MTK_CAMCORDER_PROFILE_MID_MP4 = no
MTK_BRAZIL_CUSTOMIZATION_TIM = no
MTK_BRAZIL_CUSTOMIZATION = no
MTK_BRAZIL_CUSTOMIZATION_CLARO = no
MTK_BRAZIL_CUSTOMIZATION_VIVO = no

#
# MTK APP
#
MTK_GALLERY3D_APP = yes
MTK_INPUTMETHOD_PINYINIME_APP = no
MTK_GALLERY_APP = yes
MTK_CAMERA_APP = yes
MTK_ENGINEERMODE_APP = yes
MTK_YGPS_APP = yes
MTK_LIVEWALLPAPER_APP = yes

MTK_AGPS_APP = no
# orig
#MTK_AGPS_APP = yes
MTK_ACWFDIALOG_APP = yes
MTK_LAUNCHERPLUS_APP = yes
MTK_SOUNDRECORDER_APP = no
MTK_DM_APP = no
MTK_ANDROIDFACTORYMODE_APP = yes
MTK_VIDEOWIDGET_APP = yes
MTK_FILEMANAGER_APP = yes
MTK_APKINSTALLER_APP = no
MTK_SMSREG_APP = no

MTK_VLW_APP = no
# orig
#MTK_VLW_APP = yes

MTK_VIDEOPLAYER_APP = yes
MTK_CALENDAR_IMPORTER_APP = yes
MTK_THEMEMANAGER_APP = yes
MTK_DRM_APP = yes
MTK_WEATHER_PROVIDER_APP = no
MTK_LOG2SERVER_APP = no
MTK_VIDEOPLAYER2_APP = yes
MTK_WEATHER_WIDGET_APP = no
MTK_ENGINEERMODE_INTERNAL_APP = yes
MTK_WORLD_CLOCK_WIDGET_APP = yes
MTK_DATADIALOG_APP = yes
MTK_BACKUPANDRESTORE_APP = yes
MTK_TODOS_APP = yes
# LuckAs add
# Disable MTK Bluetooth stack and enable Broadcom Bluetooth
BCM_BT_SUPPORT = yes
MTK_BT_SUPPORT = no
MTK_BT_CHIP = BCM4330
MTK_BT_21_SUPPORT = no
MTK_BT_30_HS_SUPPORT = no
MTK_BT_30_SUPPORT = no
MTK_BT_40_SUPPORT = no
MTK_BT_FM_OVER_BT_VIA_CONTROLLER = no
MTK_BT_PROFILE_A2DP = no
MTK_BT_PROFILE_AVRCP = no
MTK_BT_PROFILE_AVRCP14 = no
MTK_BT_PROFILE_BIP = no
MTK_BT_PROFILE_BPP = no
MTK_BT_PROFILE_DUN = no
MTK_BT_PROFILE_FTP = no
MTK_BT_PROFILE_HFP = no
MTK_BT_PROFILE_HIDH = no
MTK_BT_PROFILE_MANAGER = no
MTK_BT_PROFILE_MAPC = no
MTK_BT_PROFILE_MAPS = no
MTK_BT_PROFILE_OPP = no
MTK_BT_PROFILE_PAN = no
MTK_BT_PROFILE_PBAP = no
MTK_BT_PROFILE_PRXM = no
MTK_BT_PROFILE_PRXR = no
MTK_BT_PROFILE_SIMAP = no
MTK_BT_PROFILE_SPP = no
MTK_BT_PROFILE_TIMEC = no
MTK_BT_PROFILE_TIMES = no

# LuckAs end

####config BT feature###########
# please do not modify these options

# orig
#MTK_BT_21_SUPPORT = yes
#MTK_BT_30_SUPPORT = yes
#MTK_BT_30_HS_SUPPORT = yes
#MTK_BT_40_SUPPORT = yes
#MTK_BT_FM_OVER_BT_VIA_CONTROLLER = yes
#MTK_BT_PROFILE_OPP = yes
#MTK_BT_PROFILE_SIMAP = yes
#MTK_BT_PROFILE_PRXM = no
#MTK_BT_PROFILE_PRXR = no
#MTK_BT_PROFILE_HIDH = yes
#MTK_BT_PROFILE_FTP = yes
#MTK_BT_PROFILE_PBAP = yes
#MTK_BT_PROFILE_MANAGER = yes
#MTK_BT_PROFILE_BPP = yes
#MTK_BT_PROFILE_BIP = yes
#MTK_BT_PROFILE_DUN = yes
#MTK_BT_PROFILE_PAN = yes
#MTK_BT_PROFILE_HFP = yes
#MTK_BT_PROFILE_A2DP = yes
#MTK_BT_PROFILE_AVRCP = yes
#MTK_BT_PROFILE_AVRCP14 = yes
#MTK_BT_PROFILE_TIMEC = no
#MTK_BT_PROFILE_TIMES = no
#MTK_BT_PROFILE_MAPS = yes
#MTK_BT_PROFILE_MAPC = yes
#MTK_BT_PROFILE_SPP = yes
####config BT feature end ####

#
# SMS add for IME feature
#
MTK_SMS_FILTER_SUPPORT = yes
MTK_SMS_NATIONAL_LANGUAGE_SUPPORT = no
MTK_SMS_TURKISH_TABLE_ALWAYS_SUPPORT = yes

MTK_IME_HANDWRITING_ENGINE = none
MTK_IME_INPUT_ENGINE = none
MTK_IME_SUPPORT = no
MTK_IME_RUSSIAN_SUPPORT = no
MTK_IME_ENGLISH_SUPPORT = yes
MTK_IME_PINYIN_SUPPORT = no
MTK_IME_STROKE_SUPPORT = no
MTK_IME_ZHUYIN_SUPPORT = no
MTK_IME_HANDWRITING_SUPPORT = no
MTK_IME_FRENCH_SUPPORT = no
MTK_IME_GERMAN_SUPPORT = no
MTK_IME_SPANISH_SUPPORT = no
MTK_IME_ITALIAN_SUPPORT = no
MTK_IME_PORTUGUESE_SUPPORT = no
MTK_IME_TURKISH_SUPPORT = no
MTK_IME_MALAY_SUPPORT = no
MTK_IME_HINDI_SUPPORT = no
MTK_IME_ARABIC_SUPPORT = no
MTK_IME_THAI_SUPPORT = no
MTK_IME_VIETNAM_SUPPORT = no
MTK_IME_INDONESIAN_SUPPORT = no

#
# SNS   
#
MTK_SNS_SUPPORT = yes
MTK_SNS_KAIXIN_APP = yes
MTK_SNS_RENREN_APP = yes
MTK_SNS_FACEBOOK_APP = yes
MTK_SNS_FLICKR_APP = yes
MTK_SNS_TWITTER_APP = yes
MTK_SNS_SINAWEIBO_APP = yes
MTK_SNS_SINAWEIBO_TEST = no


